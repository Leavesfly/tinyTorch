...
本教程深入讲解 tinyTorch 中的张量（NdArray）操作。

## 什么是 NdArray？

NdArray 是多维数组的数学表示，是深度学习的基础数据结构。在 tinyTorch 中，NdArray 使用扁平列表存储数据，通过 Shape 对象管理维度。
...
# 全零张量
zeros = NdArray.zeros((3, 4))

# 全一张量
ones = NdArray.ones((2, 3))

# 随机张量（正态分布）
randn = NdArray.randn((2, 2))

# 随机张量（均匀分布）
rand = NdArray.rand((3, 3))

# 单位矩阵
eye = NdArray.eye(3)

# 等差数列
arange = NdArray.arange(0, 10, 2)  # [0, 2, 4, 6, 8]
```
