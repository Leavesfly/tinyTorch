...
x = Tensor(NdArray([1.0]))
y = Tensor(NdArray([2.0]))
z = x + y
z.backward()
...
# 矩阵乘法梯度
A = Tensor(NdArray([[1.0, 2.0]]))  # (1, 2)
B = Tensor(NdArray([[3.0], [4.0]]))  # (2, 1)
C = A.matmul(B)  # (1, 1)
...
```python
from tinytorch.autograd import Function, Tensor
...
class MySquare(Function):
    ...
    def forward(self, x):
        ...
        return Tensor(result, creator=self)
...
**Q: Tensor 和 NdArray 的区别？**  
A: NdArray 只存储数据，Tensor 额外存储梯度和计算图信息。
