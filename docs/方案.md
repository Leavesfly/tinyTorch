...
graph TB
    ...
    subgraph "张量层 tinytorch.ndarr"
        TS_TENSOR[NdArray 多维数组]
        TS_SHAPE[Shape 形状管理]
...
**核心组件**：
- `NdArray`：多维数组核心类，支持创建、索引、变形、运算等操作
...
| 方法名 | 参数 | 返回值 | 说明 |
|--------|------|--------|------|
| `__init__` | data, shape, dtype | NdArray | 构造函数 |
| zeros | shape | NdArray | 创建全零张量 |
| ones | shape | NdArray | 创建全一张量 |
| randn | shape | NdArray | 创建标准正态分布随机张量 |
| reshape | new_shape | NdArray | 改变张量形状 |
| transpose | axes | NdArray | 转置张量 |
| add | other | NdArray | 张量加法 |
| mul | other | NdArray | 张量乘法 |
| matmul | other | NdArray | 矩阵乘法 |
| sum | axis, keepdims | NdArray | 求和归约 |
| mean | axis, keepdims | NdArray | 求平均值 |
...
| 方法名 | 参数 | 返回值 | 说明 |
|--------|------|--------|------|
| `__init__` | value, name, requires_grad | Tensor | 构造函数 |
| backward | retain_graph | None | 反向传播计算梯度 |
| clear_grad | - | None | 清除梯度 |
| detach | - | Tensor | 分离计算图 |
| add | other | Tensor | 变量加法 |
| mul | other | Tensor | 变量乘法 |
| matmul | other | Tensor | 矩阵乘法 |
| relu | - | Tensor | ReLU 激活 |
| sigmoid | - | Tensor | Sigmoid 激活 |
| reshape | shape | Tensor | 改变形状 |
...
sequenceDiagram
    ...
    participant Var as Tensor
    participant Func as Function
    participant NdArray as NdArray
    
    User->>Var: x.add(y)
    Var->>Func: 创建 Add 函数实例
    Func->>NdArray: forward(x.value, y.value)
    NdArray-->>Func: 返回计算结果 tensor
    Func->>Var: 创建输出 Tensor(tensor)
    Func->>Var: 设置 output.creator = self
    Func->>Func: 保存 inputs = [x, y]
    Var-->>User: 返回输出 Tensor
```
...
| 属性名 | 类型 | 说明 |
|--------|------|------|
| inputs | list[Tensor] | 输入变量列表 |
| outputs | list[Tensor] | 输出变量列表 |
...
| 方法名 | 参数 | 返回值 | 说明 |
|--------|------|--------|------|
| forward | *inputs | NdArray/list[NdArray] | 前向传播（抽象方法） |
| backward | grad_output | list[NdArray] | 反向传播（抽象方法） |
| call | *inputs | Tensor/list[Tensor] | 函数调用入口 |
...
| 方法名 | 参数 | 返回值 | 说明 |
|--------|------|--------|------|
| forward | *inputs | Tensor | 前向传播（抽象方法） |
...
│   ├── tensor/                         # ✅ 张量模块（已完成）
│   │   ├── __init__.py
│   │   ├── tensor.py                   # ✅ NdArray 核心类（完整实现）
│   │   └── shape.py                    # ✅ Shape 形状管理（完整实现）
│   │
│   ├── autograd/                       # ✅ 自动微分模块（已完成）
│   │   ├── __init__.py
│   │   ├── variable.py                 # ✅ Tensor 变量类
...
│   ├── 02_tensor_operations.md         # ✅ NdArray 操作详解（367行）
...
5. **详细的中文教程**：3 篇核心教程，覆盖 NdArray、Autograd、神经网络基础
...
sequenceDiagram
    ...
    participant V as Tensor
    participant F as Function
    participant T as NdArray
...
- **构建时机**：前向传播时动态构建
- **节点类型**：Tensor（变量节点）和 Function（函数节点）
- **边的表示**：Function 保存 inputs 和 outputs 引用
- **图的存储**：隐式存储在 Tensor 的 creator 属性中
...
| NdArray | NdArray | 多维数组核心类 |
| Shape | Shape | 形状管理 |
| Tensor | Tensor | 自动微分变量 |
...
```java
// Java
Tensor x = new Tensor(ndArray, "x");
Tensor y = x.add(new Tensor(1.0));
y.backward();
