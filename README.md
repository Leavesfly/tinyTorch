...
- 🧩 **完整实现**：包含 NdArray、自动微分、神经网络、训练框架
...
│  Tensor | Function | Operations    │
├─────────────────────────────────────┤
│    张量层 (tinytorch.tensor)         │
│   NdArray | Shape | Broadcasting      │
└─────────────────────────────────────┘
```
...
### tinytorch.utils.data - 数据加载
PyTorch 风格的数据接口：

| 组件 | 功能 |
|------|------|
| `Dataset` | Map-style 数据集基类 |
| `IterableDataset` | Iterable-style 数据集基类 |
| `DataLoader` | 批量迭代与默认 collate |
...
| [NdArray 操作详解](tutorials/02_tensor_operations.md) | 张量创建、运算、广播机制 | 1h |
...
- [NdArray 详解](tutorials/02_tensor_operations.md) - 张量操作
